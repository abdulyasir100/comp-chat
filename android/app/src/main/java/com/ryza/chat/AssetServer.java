...
public final class AssetServer extends Thread {
    ...
    private void proxy(String rawUrl, Headers hs, InputStream in, OutputStream out) throws IOException {
        ...
        if (!allowedTarget(target)) {
            write(out, 400, "application/json", "{\"error\":{\"message\":\"proxy target must be https, or http to a private/tailnet host\"}}");
            return;
        }
        try {
    ...
    /** https anywhere; plain http only to loopback / RFC1918 / Tailscale CGNAT
     *  (OmniVoice + the Claude bridge sit on the tailnet over http). Same rule
     *  as scripts/serve.py and desktop/main.js. */
    private static final java.util.regex.Pattern PRIVATE_HTTP = java.util.regex.Pattern.compile(
        "^http://(127\\.0\\.0\\.1|localhost|10\\.\\d+\\.\\d+\\.\\d+|192\\.168\\.\\d+\\.\\d+|"
        + "172\\.(1[6-9]|2\\d|3[01])\\.\\d+\\.\\d+|100\\.(6[4-9]|[7-9]\\d|1[01]\\d|12[0-7])\\.\\d+\\.\\d+)(:\\d+)?(/|$)",
        java.util.regex.Pattern.CASE_INSENSITIVE);

    static boolean allowedTarget(String t) {
        if (t == null) return false;
        return t.startsWith("https://") || PRIVATE_HTTP.matcher(t).find();
    }
