...
(function (global) {
  ...
  var Avatar = {
    ...
    loadSkin: function (skinId, cb) {
      ...
      var apply = function (skins) {
        ...
        gP.then(function (g) {
          ...
          App && App.toast((window.I18n ? I18n.tc('avatar.skinFail', 'Outfit failed to load: ') : 'Outfit failed to load: ') + e.message, true);
          cb && cb(e);
        });
      };
    ...
    _bindPointer: function () {
      ...
      /* Desktop: follow the cursor anywhere in the window (live2d-companion
         behaviour), measured against the stage so the -1..1 mapping in the
         renderers stays the same. Touch devices keep the stage-only path. */
      ...
      if (fine) {
        ...
        document.addEventListener('pointerleave', function () { Avatar._pointer.on = false; });
        window.addEventListener('blur', function () { Avatar._pointer.on = false; });
      }
    },
    ...
    setHidden: function (on) {
      /* only the drawing flag — Stage toggles #avatar-hit for the active
         renderer, otherwise hiding the idle Spine rig under a Live2D card
         switched taps and cursor-follow off for good */
