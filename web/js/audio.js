...
(function (global) {
  ...
  /* BGM sits under the ambience on the talk screen */
  var TALK_BGM_DUCK = 0.6;
  ...
  var Sound = {
    ...
    _applyRoute: function () {
      ...
      /* talk and every other in-game screen: location ambient plus the BGM
         under it. (The source game kept BGM for the world map, which
         companion-chat removed, so the talk screen was silent apart from
         ambience; the BGM slider then did nothing.) */
      Sound._playLoop(Sound.bgm, BGM.world, 'bgm', TALK_BGM_DUCK);
    ...
    applyVolumes: function () {
      if (Sound._route === 'world') {
        ...
        Sound.bgm.volume = Sound._gain('bgm') * TALK_BGM_DUCK;
      }
    },
