...
/* https anywhere; plain http only to loopback / RFC1918 / Tailscale CGNAT —
   companion-chat's OmniVoice and the Claude bridge live on the tailnet over
   http. Same rule as scripts/serve.py and the Android AssetServer. */
const PRIVATE_HTTP = /^http:\/\/(127\.0\.0\.1|localhost|10\.\d+\.\d+\.\d+|192\.168\.\d+\.\d+|172\.(1[6-9]|2\d|3[01])\.\d+\.\d+|100\.(6[4-9]|[7-9]\d|1[01]\d|12[0-7])\.\d+\.\d+)(:\d+)?(\/|$)/i;
function allowedTarget(t) {
  t = String(t || '');
  return t.startsWith('https://') || PRIVATE_HTTP.test(t);
}

async function proxyRequest(request, targetUrl) {
  if (!allowedTarget(targetUrl)) {
    return jsonError(400, 'proxy target must be https, or http to a private/tailnet host');
  }
