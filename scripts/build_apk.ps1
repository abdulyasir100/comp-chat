...
param(
  [switch]$DebugBuild,   # -DebugBuild: android:debuggable=true -> WebView remote debugging on
  [switch]$Full          # -Full: personal build with ALL of web/ (licensed Ryza media, BGM, backgrounds, private cards); never distribute
)
...
$Web = Join-Path $Root "output/web-stage"   # staged by scripts/stage_web.py below
...
$Ks = Join-Path $KsDir "companion.keystore"
if (-not (Test-Path $Ks)) {
  & (Join-Path $Jdk "bin/keytool.exe") -genkeypair -v -keystore $Ks -alias ryza `
    -keyalg RSA -keysize 2048 -validity 10000 `
    -dname "CN=Companion Chat, OU=companion-chat" -storepass ryza-chat -keypass ryza-chat | Out-Null
}
$Apk = Join-Path $Out $(if ($Full) { "CompanionChat-$Ver-full.apk" } else { "CompanionChat-$Ver.apk" })
