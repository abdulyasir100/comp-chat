...
param([switch]$Full)   # -Full: personal build with ALL of web/ (licensed Ryza media, BGM, backgrounds, private cards); never distribute
$ErrorActionPreference = "Stop"
$Root = Split-Path -Parent $PSScriptRoot
Set-Location (Join-Path $Root "desktop")
if ($Full) { "!! FULL BUILD: bundles licensed media and private cards, privacy gates skipped - for your own devices only" }
...
"== stage web/ =="
$StageArgs = @(if ($Full) { "--full" }) + @((Join-Path $Root "output/web-stage"))
python (Join-Path $PSScriptRoot "stage_web.py") @StageArgs
...
if (-not $Full) {
  ...
  python (Join-Path $PSScriptRoot "privacy_check.py") `
    (Join-Path $Root "output/web-stage") "main.js" "preload.js" "web-storage.js" "package.json" `
...
if ($Full) {
  "== FULL build: privacy gate skipped; renaming the installer so it cannot be mistaken for a release =="
  Get-ChildItem (Join-Path $Root "output/desktop*/CompanionChat-Setup-$Ver.exe") | ForEach-Object {
    $dst = Join-Path $_.DirectoryName "CompanionChat-Setup-$Ver-full.exe"
    Move-Item -Force $_.FullName $dst; "Built: $dst"
  }
} else {
  ...
  Get-ChildItem (Join-Path $Root "output/desktop*/CompanionChat-Setup-$Ver.exe") | ForEach-Object { "Built: $($_.FullName)" }
}
