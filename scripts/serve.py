...
# https anywhere; plain http only to loopback / RFC1918 / Tailscale CGNAT (OmniVoice on 100.x)
_PRIVATE_HTTP = _re.compile(
    r"^http://(127\.0\.0\.1|localhost|10\.\d+\.\d+\.\d+|192\.168\.\d+\.\d+|"
    r"172\.(1[6-9]|2\d|3[01])\.\d+\.\d+|100\.(6[4-9]|[7-9]\d|1[01]\d|12[0-7])\.\d+\.\d+)(:\d+)?(/|$)", _re.I)


def _allowed_target(target: str) -> bool:
    return target.startswith("https://") or bool(_PRIVATE_HTTP.match(target))
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def _proxy_get(self, parsed):
        ...
        if not _allowed_target(target):
            self.send_error(400, "proxy target must be https, or http to a private/tailnet host")
            return
        try:
