/* Boot smoke: run the real App.init() against a fake DOM whose element ids
   come from the actual index.html, with real config/i18n/game/quests
...
sandbox.Onboarding = {
  ...
  skip() {}, next() {}, birthdayPicker() { return makeEl('bday'); }
};
...
for (const f of ['util.js', 'config.js', 'i18n.js', 'api.js', 'memory.js',
                 '../engine/store.js', '../engine/affection.js', '../engine/guard.js',
                 '../engine/reply.js', '../engine/prompt.js', '../engine/characters.js', '../engine/packfs.js', '../engine/capabilities.js', '../engine/importer.js',
                 '../engine/providers/registry.js', '../engine/providers/llm-openai.js',
                 '../engine/providers/tts-omnivoice.js', '../engine/index.js',
                 '../renderers/stage.js', '../renderers/spine.js', '../renderers/cubism.js',
                 'game.js', 'quests.js', 'world.js', 'audio.js',
...
(async () => {
  try {
    ...
    /* companion-chat: the RPG surfaces (quests, inventory, stamina HUD,
       world map, skins) are removed from the markup; their modules still load
       and must stay inert. */
    const g = sandbox.Game;
    ok(g.s.stamina > 0, 'game state initialized (module still loads)');
    ok(document.getElementById('hud-stamina') === null && document.getElementById('view-world') === null,
       'RPG surfaces are gone from the markup');
    ok(sandbox.App._el('hud-stamina') && sandbox.App._el('hud-stamina') !== document.getElementById('hud-stamina'),
       'removed elements resolve to inert stubs');
    sandbox.Quests.render(sandbox.App._el('quest-list'), {});
    sandbox.App.renderWorld();
    sandbox.App.renderSkins();
    ...
    /* log-panel lifecycle: the panel is persistent —
